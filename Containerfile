...
RUN wget https://raw.githubusercontent.com/ublue-os/nvidia/main/install.sh -O /tmp/nvidia-install.sh && \
    ...
    chmod +x /tmp/nvidia-install.sh && env FEDORA_MAJOR_VERSION="$(rpm -E %fedora)" IMAGE_NAME="silverblue" /tmp/nvidia-install.sh && \
    chmod +x /tmp/nvidia-post-install.sh && env FEDORA_MAJOR_VERSION="$(rpm -E %fedora)" IMAGE_NAME="silverblue" /tmp/nvidia-post-install.sh
